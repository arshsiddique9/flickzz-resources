        ...
        tbody.innerHTML = items.map(f => {
            ...
            return `
                <tr>
                    ...
                    <td>
                        <div class="action-btns">
                            <button class="icon-btn" data-fb-action="toggle" data-id="${f.id}" data-hidden="${!!f.hidden}" title="${f.hidden ? 'Show' : 'Hide'}"><i class="fas fa-${f.hidden ? 'eye' : 'eye-slash'}"></i></button>
                            <button class="icon-btn danger" data-fb-action="delete" data-id="${f.id}" title="Delete"><i class="fas fa-trash"></i></button>
                        </div>
                    </td>
                </tr>
            `;
                ...
                <td>
                    ${isOwnerRow ? '<span style="color:var(--text-dim);font-size:0.85rem;">Cannot modify owner</span>' : `
                        <div class="action-btns">
                            ...
                            <button class="icon-btn danger" data-user-action="delete" data-id="${u.id}" title="Delete user record"><i class="fas fa-trash"></i></button>
                        </div>
                    `}
                </td>
            </tr>
...
function renderCommentsTable() {
    ...
    tbody.innerHTML = filtered.map(c => `
        <tr>
            ...
            <td><button class="icon-btn danger" data-comment-action="delete" data-rid="${c.resourceId}" data-cid="${c.id}" title="Delete"><i class="fas fa-trash"></i></button></td>
        </tr>
    `).join('');
