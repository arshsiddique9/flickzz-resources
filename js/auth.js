...
async function ensureUserDoc(user) {
    ...
    try {
        ...
        if (!snap.exists()) {
            await setDoc(userRef, { uid: user.uid, email: user.email, displayName: user.displayName || user.email.split('@')[0], photoURL: user.photoURL || null, isAdmin: isAdminEmail(user.email), isPublisher: false, createdAt: serverTimestamp() });
            try { const { bumpPublicMembersCount } = await import('./resources-api.js'); await bumpPublicMembersCount(); } catch (e) { console.warn('[ensureUserDoc] publicStats bump skipped:', e?.message); }
        }
    } catch (err) { console.error("ensureUserDoc error:", err); }
}
...
export async function logout() { if (!auth) return; await signOut(auth); window.location.href = "index.html"; }
...
export function requireAuth(redirectTo = "login.html") {
    return new Promise((resolve) => {
        onAuthReady((state) => { if (!state.user) { sessionStorage.setItem('redirectAfterLogin', window.location.pathname); window.location.href = redirectTo; return; } resolve(state); });
    });
}

export function requireAdmin() {
    return new Promise((resolve) => { onAuthReady((state) => resolve(state)); });
}

export function requirePublisher() {
    return new Promise((resolve) => {
        onAuthReady((state) => {
            if (!state.user) { sessionStorage.setItem('redirectAfterLogin', window.location.pathname); window.location.href = 'login.html'; return; }
            if (!state.isPublisher && !state.isAdmin) { window.location.href = 'index.html'; return; }
            resolve(state);
        });
    });
}
