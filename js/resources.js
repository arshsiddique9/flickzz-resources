...
async function refresh() {
    if (!grid) return;
    // Sirf loading class add karo, grid mat khali karo
    ...
    try {
        ...
        setTimeout(() => grid.classList.remove('loading'), 50);
    }
}
...
function renderCard(r) {
    ...
    const thumb = r.thumbnail ? `<img src="${escapeHtml(r.thumbnail)}" alt="${escapeHtml(r.title)}" loading="lazy">` : `<i class="fas ${cat.icon}"></i>`;
    return `
        <a href="resource-detail.html?id=${encodeURIComponent(r.id)}" class="resource-card">
            <div class="resource-thumb">${thumb}<span class="resource-badge">${escapeHtml(cat.label)}</span>${r.featured ? '<span class="resource-featured"><i class="fas fa-star"></i> Featured</span>' : ''}</div>
